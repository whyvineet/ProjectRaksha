...
function findBestPath(startId, endId) {
    ...
    if (shortestPath.path.length === 0 || safestPath.path.length === 0) {
        return { path: [], safetyScore: 0, mode: 'no path found' };
    }
    ...
    if (safestRatio - shortestRatio >= 2) {
        ...
        for (const neighbor of getNeighbors(current)) {
            ...
            if (tentativeGScore < gScore[neighbor]) {
                ...
                if (mode === 'shortest') {
                    ...
                    fScore[neighbor] = gScore[neighbor] - (safetyScore[neighbor] * 0.5) + heuristic(neighbor, endId);
                }
...
findPathButton.addEventListener('click', () => {
    if (startCell && endCell) {
        ...
        if (result.path.length === 0) {
            updateInfoPanel('N/A', 'N/A', 'no path found');
        } else {
