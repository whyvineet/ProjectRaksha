...
// Find the best path based on the highest ratio of safety score to distance
function findBestPath(startId, endId) {
    const paths = ['shortest', 'safest'].map(mode => findPath(startId, endId, mode));
    const ratios = paths.map(path => path.safetyScore / (path.path.length - 1));
    
    const bestIndex = ratios.indexOf(Math.max(...ratios));
    return { ...paths[bestIndex], mode: paths[bestIndex].mode };
}
...
function findPath(startId, endId, mode) {
    ...
    while (openSet.length > 0) {
        ...
        if (current === endId) {
            ...
            return { path, safetyScore: safetyScore[current], mode };
        }
        ...
        for (const neighbor of getNeighbors(current)) {
            ...
            const tentativeGScore = gScore[current] + 1;  // Base distance cost
            ...
            if (tentativeGScore < gScore[neighbor]) {
                ...
                if (mode === 'shortest') {
                    const safetyPenalty = gridPoints[neighbor] < 6 ? 10 : 0;
                    fScore[neighbor] = gScore[neighbor] + heuristic(neighbor, endId) + safetyPenalty;
                } else { // 'safest'
    ...
    return { path: [], safetyScore: 0, mode };
}
...
findPathButton.addEventListener('click', () => {
    if (startCell && endCell) {
        ...
        const { path, safetyScore, mode } = findBestPath(startId, endId);
        highlightPath(path);
        updateInfoPanel(safetyScore, path.length - 1, mode);
    }
});
